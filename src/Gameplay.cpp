...
void Scenes::Gameplay::load() {
    this->flappy = Flappy(-64, 0, 8, 8, 2, 2);
    this->flappyData = FlappyData();
    this->score = Score(0, -64, FontType::BIG);
    this->background = Background();
    this->floor = Floor(0, 36);
    this->pauseButton = bn::sprite_items::ui_small_btn.create_sprite(-102, -64);
    ...
    for(int index=0; index < this->pipes.max_size(); index++) {
        this->pipes.push_back(PipeWall(
            ...
            PIPE_WALL_WIDTH,
            generateGapSize(this->random.get()), COLOR_WHITE, 
            -3));
...
void Scenes::Gameplay::leave() {
    ...
    this->flappyData.~optional();
    this->flappy.~optional();
    this->background.~optional();
    this->score.~optional();
    this->floor.~optional();
    this->pauseButton.~optional();
}
...
void Scenes::Gameplay::getReadyScene() {
    ...
    while(true) {
        ...
        this->flappy->update();
        this->floor->update();
        if(this->getReady == true) this->getReady->update();
        bn::core::update();
    }
}
void Scenes::Gameplay::gameScene() {
        ...
        for(int index = 0; index < this->pipes.size(); index++) {
            ...
            if(pipe->getX() < -SCREEN_WIDTH_HALF - PIPE_WALL_WIDTH) {
                ...
                pipe->setY(this->random->get_int(-SCREEN_HEIGHT_HALF + 16, SCREEN_HEIGHT_HALF - 86));
                pipe->setGapSize(generateGapSize(this->random.get()));
                pipe->setScoredFlag(false);
            }
        }
        ...
        if(this->getReady == true) {
            ...
            if(this->getReady->isDone()) this->getReady.reset();
        }
        this->floor->update();
        bn::core::update();
}
